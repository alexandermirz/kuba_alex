...
asd_full_df$Gender <-
  ifelse(asd_full_df$Gender == 1, yes = "M", no = "F") #changing the gendertypes 1 and 2 to F and M

asd_full_df$Diagnosis <- 
  ifelse(asd_full_df$Diagnosis == "A", yes = "ASD", no = "TD") #changing diagnosis types from A and B to ASD and TD 

###Final save####
write.csv(asd_full_df, file = "asd_full_df", row.names = FALSE)
##################
```
...
for (i in asd_full_df$SUBJ){
  sub=filter(asd_full_df, VISIT==i)
  mean=mean(sub$tokens_CHI)
  print(mean)
  visit_tokens <- rbind(visit_tokens, mean)

#### Attempting to duplicate the verbal IQ for each participant from 1st visit ### 
ExLaRaw_1st <- vector("numeric") 
...
for (i in asd_full_df$SUBJ){
  ...
  ExLaRaw_1st <- rbind(ExLaRaw_1st, values)
}

### Appending data to dataframe
asd_full_df <- mutate(asd_full_df, ExLaRaw_1st)


#### Attempting to duplicate the non-verbal IQ for each participant from 1st visit ### 
MR_1st <- vector("numeric") 
...
for (i in asd_full_df$SUBJ){
  ...
  print(first$MR[1])
  values=first$MR[1]
  MR_1st <- rbind(MR_1st, values)
}

### Appending data to dataframe
asd_full_df <- mutate(asd_full_df, MR_1st)

#### Attempting to duplicate ADOS for each participant from 1st visit ### 
ADOS_1st <- vector("numeric") 
...
for (i in asd_full_df$SUBJ){
  ...
  ADOS_1st <- rbind(ADOS_1st, values)
}

### Appending data to dataframe
asd_full_df <- mutate(asd_full_df, ADOS_1st)
...
subset(asd_full_df, !duplicated(SUBJ)) 
subset(asd_full_df, SUBJ=="AD")
